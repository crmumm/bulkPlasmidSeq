...
python bulkPlasmidSeq Porechop -i ${READS} -r ${REFERENCE} -o ${OUTPUTDIR} -t ${THREADS}

#For Porechop with filtering and screenshots
python bulkPlasmidSeq Porechop -i ${READS} -r ${REFERENCE} -o ${OUTPUTDIR} -t ${THREADS} --filter \
